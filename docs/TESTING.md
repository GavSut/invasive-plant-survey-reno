...
|---|---|---|
| Student release | App/cache `2.1.0`, protocol `2.0.0`, 30 segments, six cells per segment, 180 total cells, four distinct statuses, 23-species selector, backup compatibility, retained guide files, and no visible student guide route | Open the deployed Pages site online once, then verify draft entry/reopen offline on real phones |
...
4. Before login, inspect the page and accessibility tree: no class names, counts, record IDs, observers, coordinates, photos, stale map layers, or chart labels may be exposed.
5. Exercise filters, sort, pagination, select-page, select-all-filtered, clear selection, row click/keyboard activation, map selection, dialogs, empty/error/partial states, and session expiry.
6. Test the dashboard at 200% zoom, with keyboard only, at tablet width, and at a narrow phone width. Confirm focus, scrolling, dialogs, tables, maps, and download controls remain usable.

...
test("release 2.1.2 changes the app/cache version without changing ecological protocol v2", () => {
  assert.equal(packageJson.version, "2.1.2");
  assert.equal(CONFIG.appVersion, "2.1.2");
  assert.match(worker, /CACHE_NAME\s*=\s*"invasive-transect-app-v2\.1\.2"/);
  assert.equal(PROTOCOL_VERSION, "2.0.0");
  assert.equal(SCHEMA_VERSION, 2);
  assert.equal(TOTAL_CELLS, 180);
  const record = createTransect();
  assert.equal(record.appVersion, "2.1.2");

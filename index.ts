...
function isProtocolV2Payload(payload: any) {
  if (!payload || payload.protocolVersion !== "2.0.0"
    ...
    || !["reno-2026.1", "reno-2026.2"].includes(payload.speciesListVersion)) return false;
...
function applyFilters(query: any, filters: JsonRecord) {
  ...
  if (filters.speciesCodes !== undefined) {
    if (!Array.isArray(filters.speciesCodes) || filters.speciesCodes.length < 1 || filters.speciesCodes.length > TARGET_CODES.size) {
      throw new ApiError(400, "invalid_filters", `speciesCodes must contain 1-${TARGET_CODES.size} target codes.`);
    }

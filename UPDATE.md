# Deploy release 2.3.2

This checklist upgrades the existing **Invasive Plant Survey Reno** installation from app `2.0.0` to `2.3.2`. It includes the instructor dashboard and consolidated student layout. Each side-level **No Target Species** shortcut now applies immediately to incomplete cells without opening a confirmation dialog; the segment-wide six-cell shortcut still confirms, and NS remains available only in individual-cell editing. It does **not** change protocol `2.0.0`, schema version `2`, the 180-cell geometry, the 23-species catalog, classes, memberships, student ownership, submitted payloads, GPS fields, GeoJSON exports, or private photographs.
...
- the extracted v2.3 GitHub web-update release with `index.html` at its root;
...
1. Open the [student app](https://gavsut.github.io/invasive-plant-survey-reno/) online. Confirm app `2.3.2`, 30 segments, 180 cells, 23 selectable targets, and no visible field-guide link or prompt.
